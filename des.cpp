...
private:
    ...
    string feistel_rounds(string left, string right, bool is_decrypt) {
        for (int i = 0; i < 16; i++) {
...
public:
    ...
    string encrypt(const string& input) {
        ...
        string combined_text = feistel_rounds(left, right, false);
    ...
    string decrypt(const string& input) {
        ...
        string left = perm.substr(0, 32);
        string right = perm.substr(32, 32);
        string combined_text = feistel_rounds(left, right, true);
        return inverse_initial_permutation(combined_text);
    }
};

string zero_pad(string text) {
    int rem = text.length() % 64;
    if (rem != 0) {
        text.append(64 - rem, '0');
    }
    return text;
}

string des_process(string text, string key, bool is_decrypt) {
    text = zero_pad(text);
...
string triple_des_process(string text, string k1, string k2, string k3, bool is_decrypt) {
    text = zero_pad(text);
...
int main() {
    ...
    if (mode == 1 || mode == 2) {
        string text, key; cin >> text >> key;
        cout << des_process(text, key, mode == 2) << endl;
    } else if (mode == 3 || mode == 4) {
        string text, k1, k2, k3; cin >> text >> k1 >> k2 >> k3;
        cout << triple_des_process(text, k1, k2, k3, mode == 4) << endl;
    }
    return 0;
}